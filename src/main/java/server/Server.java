...
public class Server {
    ...
    private void handleOne(Socket sock) throws IOException {
        ...
        int endCenter = mMatrixComputer.getmSurroundings().lookupNearest(destLat, destLon);
        try {
            srcPaths = mMatrixComputer.set2Set(srcCircle.mFirst, srcCircle.mSecond, false, endCenter, RADIUS);
            destPaths = mMatrixComputer.set2Set(destCircle.mSecond, destCircle.mFirst, false, endCenter, RADIUS);
            interPaths = mMatrixComputer.set2Set(srcCircle.mSecond, destCircle.mSecond, true, endCenter, RADIUS);
        } catch (Exception e) {
            e.printStackTrace();
        }
